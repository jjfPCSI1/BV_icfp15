...
Pour maintenir la valeur de $r$ à jour lors des mouvements, il suffit de remarquer qu'une rotation 
revient à ajouter $\overline{\pm 1}$ dans $G_r(p)$. Il suffit donc de remplacer
$r$ par $(r \pm 1) \mod k(p)$.

\subsection{Tableau de positions}
\label{par:tab_pos}
Comme dans la plupart des parcours de graphe on va être amener à maintenir un
...
booléen indexé par l'identifiant unique vu en~\ref{par:tab_pos}.
...
A l'issue du parcours précédent on obtient une liste de positions verrouillable
...
\begin{itemize}
    ...
    \item\textbf{la différence entre la hauteur du plateau et la position de la plus haute 
        ...
        la compactification du plateau en donnant un bonus aux verrouillage
...
Une amélioration rapide de l'algorithme consisterait dans un second temps à
regarder pour chaque cellule si on peut l'atteindre par une phrase spéciale à
partir d'une position découverte. Ce genre d'optimisation peut paraitre
évidente à la lecture de cet article, d'autant en constatant sur la
