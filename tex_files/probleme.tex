...
présenté dans la figure~\ref{fig:hexcoord1}
\begin{figure}
...
dans la grille ne sont pas proportionnels à des déplacements géométriques. 
En effet, si on souhaite effectuer une translation d'un cran en bas à droite (ce qui sera défini comme le mouvement
...
\begin{description}
    ...
    \item[$\mathcal{O}$] est l'ensemble des cases déjà occupées ;
    \item[$N$] est le nombre de pièces à placer lors de chaque partie ;
    \item[$\overline{g} = (g_1,\dots,g_m)$] sont les germes des parties à jouer
        ;
    \item[$\overline{p} = (p_1,\dots,p_n)$] sont les différents types de pièces disponibles.
\end{description}
...
Un mouvement est valide si toutes les cellules occupées après après l'avoir
effectué sont libres.

Lorsque l'on effectue un mouvement invalide, la pièce est verrouillée sur le
...
Lorsque après avoir verrouillé une pièce, des rangées du plateau sont
entièrement occupées,
elles sont remplacées par des cases libres et les rangées situées au dessus descendent d'un
cran (les cases des rangées paires effectuent donc un mouvement $\sSW$ et les autres un 
mouvement $\sSE$).
...
Une part importante du score final pour une partie est déterminée par la fréquence d'apparition 
...
vis-à-vis de son pivot, certaines phrases effectueront toujours un mouvement
invalide  (en raison de la règle de non répétition présentée au
paragraphe~\ref{par:stuttering}).
Par exemple pour une pièce triviale réduite à une seule case contenant son pivot aucune des phrases contenant 
...
des 18 phrases spéciales\footnote{Dont 5 dans la dernière heure grâce à
d'obscurs échanges avec deux équipes russes rencontrées sur le canal {\tt \#icfp-contest} 
...
Pour ne pas que l'on puisse exploiter les phrases spéciales en créant des suites
